...
Add the PureMVC_AS3 gem to your Gemfile:
	...
	<pre><code>gem "puremvc-as3", ">= 2.0.4.pre"</code></pre>

Add the PureMVC_AS3 Library dependency to your Rakefile:
	
	<pre><code>library "puremvc-as3"</code></pre>
	
The swc, src, and asdocs are all available in the latest release. To include only select assets, use respective

	<pre><code>library "puremvc-as3" :swc</code></pre>
	<pre><code>library "puremvc-as3" :src</code></pre>
	<pre><code>library "puremvc-as3" :asdocs</code></pre>

Enter this shell command to resolve gem dependency: 

	<pre><code>bundle install vendor</code></pre>
	
	(installs gems to ./vendor directory)

h3. Ruby Gem

The gem can be installed independently of a project entering this command:

	<pre><code>gem install puremvc-as3 --pre</code></pre>

h3. TODO

Generators (currently in progress)

Support for Multi-Core 

Add Generator/Gem hooks for additional popular PureMVC libraries. (fabrication)

h3. Generators

	<pre><code>puremvc-as3 ProjectName</code></pre>

	* Standard PureMVC source folders
	* Gemfile and Rakefile
	* Main project class
	* ApplicationFacade class
	* StartupCommand class
	* Compile-able out-of-the-box
